    ...
    >
      ...
      <div className="divide-y divide-white/5 max-h-96 overflow-y-auto">
        {stats.dayStats.slice().reverse().map((day) => {
          const dayTransactions = day.transactions.filter(t => t.usd < 0);
          return (
            <div key={day.date}>
              <button
                className="w-full flex items-center justify-between px-6 py-3 hover:bg-white/5 transition-colors text-left active:bg-white/10"
                onClick={() => setExpanded(expanded === day.date ? null : day.date)}
              >
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <span className="text-white/30 text-xs mono flex-shrink-0">{formatDateFull(day.date)}</span>
                  {day.cities.length > 0 && (
                ...
                <div className="flex items-center gap-3 flex-shrink-0 ml-2">
                  ...
                  <span className="mono text-sm font-bold text-white whitespace-nowrap">${day.total.toFixed(2)}</span>
